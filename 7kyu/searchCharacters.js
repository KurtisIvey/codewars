...
// we receive object that has the characters property
// we verify if it has the key specified. we then iterate through characters array until finding character obj with matching key val pair
// if found, push char to foundCharacters
...
function getCharacters(obj, key, val) {
  ...
  // Iterate over each character object in the characters array, if not returns empty foundCharacters array
  for (var i = 0; i < obj.characters.length; i++) {
    var character = obj.characters[i];

    // Convert the current character's key value and the provided value to lowercase
    var characterValue = character[key].toLowerCase();
    var searchValue = val.toLowerCase();

    // Check if the character's key value exactly matches the provided value (case-insensitive)
    if (characterValue === searchValue) {
